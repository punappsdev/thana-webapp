  ...
  try {
    const saved = await (async () => {
      switch (d.resource) {
        ...
        case "category-attributes": { if (typeof d.categoryId !== "number" || typeof d.attributeId !== "number") throw new Error("กรุณาเลือกหมวดหมู่และคุณลักษณะ"); const data = { categoryId: d.categoryId, attributeId: d.attributeId, isFilter: d.isFilter === "on", isRequired: d.isRequired === "on", sortOrder: d.sortOrder }; return id ? prisma.categoryAttribute.update({ where: { id }, data }) : prisma.categoryAttribute.create({ data }); }
      }
    })();
