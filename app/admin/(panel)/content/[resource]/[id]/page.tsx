...
import { getContentCategoryOptions, getContentRecord, getPromotionTargetOptions } from "@/lib/admin/content-data";
...
export default async function EditContentPage({ params }: { params: Promise<{ resource: string; id: string }> }) {
  ...
  const [record, categories, targetCategories] = await Promise.all([
    getContentRecord(resource, Number(id)),
    getContentCategoryOptions(config.categoryKind),
    config.hasProductTargeting ? getPromotionTargetOptions() : [],
  ]);
  if (!record) notFound();
  return <ContentForm config={config} record={record} categories={categories} targetCategories={targetCategories} />;
}
