...
export default async function AdminContentPreview({ params, searchParams }: { params: Promise<{ resource: string; id: string }>; searchParams: Promise<{ locale?: string }> }) {
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    <Card><CardContent className="p-6 md:p-10">
      ...
      {config.bodyKind === "rich" ? <div className="mt-8 space-y-4 font-body-md [&_h2]:font-headline-md [&_h3]:font-headline-sm [&_ol]:list-decimal [&_ol]:pl-6 [&_ul]:list-disc [&_ul]:pl-6 [&_blockquote]:border-l-4 [&_blockquote]:border-primary [&_blockquote]:pl-4 [&_hr]:my-6 [&_hr]:border-t [&_hr]:border-border [&_a]:text-primary [&_a]:underline [&_a]:underline-offset-2 [&_strong]:font-semibold [&_strong]:text-foreground [&_em]:italic [&_u]:underline [&_s]:line-through [&_img]:my-4 [&_img]:rounded-lg" dangerouslySetInnerHTML={{ __html: sanitizeRichHtml(body) }} /> : <p className="mt-8 whitespace-pre-line font-body-md text-muted-foreground">{body || "ยังไม่มีคำอธิบายในภาษานี้"}</p>}
