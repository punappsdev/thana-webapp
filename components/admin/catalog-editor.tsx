...
export function CatalogEditor({ resource, edit, categories, attributes }: { resource: CatalogResource; edit?: Record<string, unknown>; categories: Option[]; attributes: Option[] }) {
  ...
  const bilingual = !["brands", "attribute-values", "category-attributes"].includes(resource);
  return <Card><CardHeader><CardTitle className="font-headline-sm">{edit ? "แก้ไขรายการ" : "เพิ่มรายการใหม่"}</CardTitle></CardHeader><CardContent><form action={action} className="grid gap-4 md:grid-cols-2">
    ...
    {resource === "subcategories" || resource === "category-attributes" ? <div className="space-y-2"><Label className="font-label-md">หมวดหมู่</Label><AdminSelect name="categoryId" defaultValue={value("categoryId") || undefined} placeholder="เลือกหมวดหมู่" className="w-full" options={categories.map((x) => ({ value: String(x.id), label: x.nameTh }))} /></div> : null}
    {resource === "attribute-values" || resource === "category-attributes" ? <div className="space-y-2"><Label className="font-label-md">คุณลักษณะ</Label><AdminSelect name="attributeId" defaultValue={value("attributeId") || undefined} placeholder="เลือกคุณลักษณะ" className="w-full" options={attributes.map((x) => ({ value: String(x.id), label: x.nameTh }))} /></div> : null}
    ...
    {resource === "brands" ? <>{field("logo", "URL โลโก้", false)}{field("websiteUrl", "เว็บไซต์", false, "url")}</> : null}
    {["categories", "subcategories"].includes(resource) ? field("coverImage", "URL รูปปก", false) : null}
    ...
    {["categories", "subcategories", "attributes", "attribute-values", "category-attributes"].includes(resource) ? field("sortOrder", "ลำดับ", false, "number") : null}
    ...
    {resource === "category-attributes" ? <><label className="flex items-center gap-2 font-label-md"><input type="checkbox" name="isFilter" defaultChecked={checked("isFilter", true)} />ใช้เป็นตัวกรอง</label><label className="flex items-center gap-2 font-label-md"><input type="checkbox" name="isRequired" defaultChecked={checked("isRequired")} />จำเป็นต้องกรอก</label></> : null}
