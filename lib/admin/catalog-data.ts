...
export async function getCatalogRows(resource: CatalogResource, input: { query?: string; page?: number } = {}) {
  ...
  switch (resource) {
    ...
    case "attributes": { const where = input.query ? { OR: [{ nameTh: { contains: input.query } }, { nameEn: { contains: input.query } }, { slug: { contains: input.query } }] } : {}; return Promise.all([prisma.attribute.findMany({ where, ...paging, orderBy: [{ sortOrder: "asc" }, { nameTh: "asc" }], include: { _count: { select: { values: true, categories: true } } } }), prisma.attribute.count({ where })]); }
    ...
    case "category-attributes": { const where = input.query ? { OR: [{ category: { nameTh: { contains: input.query } } }, { attribute: { nameTh: { contains: input.query } } }] } : {}; return Promise.all([prisma.categoryAttribute.findMany({ where, ...paging, orderBy: [{ categoryId: "asc" }, { sortOrder: "asc" }], include: { category: { select: { nameTh: true } }, attribute: { select: { nameTh: true } } } }), prisma.categoryAttribute.count({ where })]); }
  }
  })();
